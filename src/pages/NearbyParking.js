...
function NearbyParking() {
  ...
  useEffect(() => {
    const fetchParkingsFromPujas = async () => {
      try {
        ...
        let extractedParkings = [];
        pujaList.forEach(p => {
          if (p.parking) {
            extractedParkings.push({
              _id: p._id + "_parking",
              spotName: p.parking,
              location: p.location || "Bankura",
              forPandal: p.name,
              mapLink: p.parkingMap || ""
            });
          }
        });

        setParkings(extractedParkings);
        setFilteredList(extractedParkings);
      } catch (err) {
        console.error("Failed to load parking spots from pujas:", err);
      }
    };

    fetchParkingsFromPujas();
  }, []);

  const handleSearch = () => {
    if (!searchTerm.trim()) {
      setFilteredList(parkings);
      return;
    }
    ...
    const filtered = parkings.filter(
      (item) =>
        ...
        (item.spotName && item.spotName.toLowerCase().includes(query)) ||
        (item.forPandal && item.forPandal.toLowerCase().includes(query))
    );

    setFilteredList(filtered.length > 0 ? filtered : parkings);
  };
  ...
  return (
    <main 
      className="utility-page parking-page"
      style={{
        backgroundImage: `linear-gradient(135deg, rgba(26, 32, 33, 0.86), rgba(101, 48, 32, 0.7)), url("/Gallery/Parking-bg.jpg")`
      }}
    >
      <div className="utility-overlay"></div>
      <div className="utility-content">
        <div className="utility-header">
          <p className="utility-badge">🚗 VEHICLE PARKING</p>
          <h1>Nearby <span>Parking Zones</span></h1>
          <p>Find dedicated parking spots and avoid traffic near major pandals. Click bubbles for route!</p>
        </div>
        ...
        <div className="utility-search-box">
          <input
            type="text"
            placeholder="Search by area or pandal name..."
            value={searchTerm}
        ...
        <div className="utility-grid bubble-popup-grid" style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: "20px", marginTop: "20px" }}>
          {filteredList.length === 0 ? (
            <p className="no-data" style={{ color: "#fff", textAlign: "center", gridColumn: "1 / -1" }}>
              🚫 কোনো পার্কিং জোন পাওয়া যায়নি। অ্যাডমিন ড্যাশবোর্ড থেকে যোগ করুন।
            </p>
          ) : (
            filteredList.map((item, index) => (
              ...
              >
                <div className="utility-card-icon bubble-icon" style={{ fontSize: "24px", marginBottom: "10px" }}>🅿️</div>
                <div className="utility-card-body bubble-info">
                  <h3 style={{ color: "#ffd36a", fontSize: "18px", marginBottom: "8px" }}>{item.spotName}</h3>
                  <p className="utility-subtext" style={{ color: "#e5e7eb", fontSize: "14px", marginBottom: "5px" }}>📍 Area: {item.location}</p>
                  {item.forPandal && <p className="utility-tag" style={{ color: "#d1d5db", fontSize: "13px", marginBottom: "15px" }}>🛕 Dedicated for: {item.forPandal}</p>}
                  ...
                  <button
                    ...
                    onClick={() => {
                      const destination = encodeURIComponent(`${item.spotName}, ${item.location}`);
                      window.open(`https://www.google.com/maps/dir/?api=1&destination=${destination}`, "_blank");
                    }}
                  >
                    🗺️ Navigate to Parking ➜
                  </button>
                </div>
              </div>
            ))
